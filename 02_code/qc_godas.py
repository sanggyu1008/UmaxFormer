...
EXCLUDE_BASENAME = "pottmp.198001-202512.nc"
...
def infer_var_from_filename(ncfile: Path) -> str:
    # 예: mlotst.198001-202512.nc -> mlotst
    return ncfile.name.split(".")[0]
...
def find_data_var(ds: xr.Dataset, expected_var: str, time_name: str | None) -> str | None:
    # 1순위: expected_var와 동일한 변수명
    if expected_var in ds.data_vars:
        return expected_var

    # 2순위: time 차원을 가지는 대표 변수
    candidates = []
...
def analyze_file(ncfile: Path) -> dict[str, Any]:
    expected_var = infer_var_from_filename(ncfile)
    ...
    try:
        ...
        if expected_var not in OCEAN_VARS:
            row["status"] = "ERROR"
            row["note"] = f"unexpected filename variable: {expected_var}"
            ds.close()
            return row
        ...
        # 해양 변수는 NaN 자체보다 "mask 변화"를 경고로 봄
...
def gather_ncfiles(root: Path) -> list[Path]:
    files = []
    for p in sorted(root.glob("*.nc")):
        if p.name == EXCLUDE_BASENAME:
            continue
        files.append(p)
    return files
...
def main():
    parser = argparse.ArgumentParser(
        description="QC GODAS nc files in one directory (non-recursive), excluding pottmp.198001-202512.nc"
    )
    parser.add_argument(
        "root",
        nargs="?",
        default=".",
        help="directory containing GODAS nc files (default: current directory)"
    )
    parser.add_argument(
        "--csv",
        default="godas_qc_summary.csv",
    ...
    ncfiles = gather_ncfiles(root)
    if not ncfiles:
        raise SystemExit(
            f"[ERROR] no .nc files found in {root} "
            f"(excluding {EXCLUDE_BASENAME})"
        )
