...
def _time_to_yyyymm(t) -> str:
    # cftime.DatetimeNoLeap / cftime.Datetime360Day / pandas.Timestamp 모두 대응
    if hasattr(t, "year") and hasattr(t, "month"):
...
def assert_same_grid(ds: xr.Dataset, ref: xr.Dataset | None, label: str) -> None:
    ...
    for coord in ["lat", "lon"]:
        ...
        if a.shape != b.shape or not np.allclose(a, b, equal_nan=True):
            raise ValueError(
                f"Grid mismatch in {label} for coord={coord}: {a.shape} vs {b.shape}"
            )
...
def merge_files(file_map: dict[str, Path], out_path: Path, fillna_mode: str = "none") -> None:
...
def build_cmip6_file_map(monthly_dir: Path) -> dict[str, Path]:
    ...
    # 우선순위 1: 최종 1x2 anomaly 파일
    cmip6_files = sorted(monthly_dir.glob("*.anom_1x2.nc"))
    if not cmip6_files:
        # 우선순위 2: monthly 폴더 내 일반 nc
        cmip6_files = sorted(monthly_dir.glob("*.nc"))
...
def run_reanalysis(base_root: Path, out_root: Path, fillna_mode: str) -> None:
    ...
    if oras5_dir.is_dir() and era5_dir.is_dir():
        try:
            ...
            out_path = out_root / "input_oras5_era5.nc"
            merge_files(fmap, out_path, fillna_mode=fillna_mode)
        except Exception as e:
    ...
    if godas_dir.is_dir() and era5_dir.is_dir():
        try:
            ...
            out_path = out_root / "input_godas_era5.nc"
            merge_files(fmap, out_path, fillna_mode=fillna_mode)
        except Exception as e:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Merge monthly ENSO anomaly files into multivariate input NetCDF files"
    )
    p.add_argument(
        "--inroot",
        type=Path,
...
def main() -> None:
    ...
    if args.mode in {"all", "reanalysis"}:
        run_reanalysis(args.inroot, args.outroot, fillna_mode=args.fillna)
