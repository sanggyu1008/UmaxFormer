...
SOURCE_DEFAULTS: dict[str, SourceDefaults] = {
    ...
    "soda": SourceDefaults(
        in_dir=Path("/mnt/d/project/01_ENSO/01_data/01_raw/soda"),
        out_dir=Path("/mnt/d/project/01_ENSO/01_data/02_interim/soda"),
        target_vars=("mlotst", "ohc300", "sos", "tos", "uos", "vos"),
        use_finite_fill_before_detrend=True,
        apply_setmissval_after_remap=True,
    ),
    "20crv2": SourceDefaults(
        in_dir=Path("/mnt/d/project/01_ENSO/01_data/01_raw/20crv2"),
        out_dir=Path("/mnt/d/project/01_ENSO/01_data/02_interim/20crv2"),
        target_vars=("psl", "uas", "vas"),
        use_finite_fill_before_detrend=True,
        apply_setmissval_after_remap=True,
    ),
...
def build_fixed_source_file_map(source: str, in_dir: Path, selected_vars: set[str]) -> dict[str, str]:
    if source == "era5":
        return ERA5_FILES
    if source == "oras5":
        return ORAS5_FILES
...
def jobs_for_fixed_source(
    ...
    jobs: list[Job] = []
    file_map = build_fixed_source_file_map(source, in_dir, selected_vars)
...
def normalize_args(args: argparse.Namespace) -> tuple[str, Path, Path, Path, set[str], int, np.float32, bool, bool, int]:
