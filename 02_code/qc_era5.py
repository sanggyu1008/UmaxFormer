...
# 고정 설정
# ============================================================
DEFAULT_ROOT = Path("/mnt/d/project/01_ENSO/01_data/01_raw/era5")
...
def check_time_bounds_continuity(ds: xr.Dataset, time_name: str | None) -> tuple[int, str]:
    ...
    try:
        ...
        left = arr[:-1, 1]
        right = arr[1:, 0]
        bad = np.where(~np.isclose(left, right, equal_nan=True))[0]
...
def compute_nan_stats(
    ...
    """
    작은 배열은 기존처럼 한 번에 처리하고,
    큰 배열(예: ERA5 daily)은 time block 단위로 streaming 처리한다.
    """
    ...
    # 1) 작은 배열: 기존 방식
    # ------------------------------------------------------------
    if est_nbytes <= 2 * 1024**3:  # 2 GiB 이하
        if other_dims:
...
def analyze_target(root: Path, target: dict[str, str]) -> dict[str, Any]:
    ...
    try:
        ...
        if time_name is None:
            row["status"] = "ERROR"
            row["note"] = (row["note"] + "; " if row["note"] else "") + "time coordinate not found"
            ds.close()
            return row
        ...
        if actual_var is None:
            row["status"] = "ERROR"
            row["note"] = (row["note"] + "; " if row["note"] else "") + f"data variable not found (expected={expected_var})"
            ds.close()
            return row
        ...
        detected_freq = infer_frequency_from_decoded(decoded)
        row["detected_freq"] = detected_freq
        ...
        if detected_freq not in ("", "unknown") and detected_freq != expected_freq:
            warn_reasons.append(f"expected_freq={expected_freq}, detected_freq={detected_freq}")
        ...
        row["status"] = "ERROR"
        row["note"] = (row["note"] + "; " if row["note"] else "") + f"{type(e).__name__}: {e}"
        return row


def main():
    parser = argparse.ArgumentParser(
        description="QC ERA5 files: uas.day / uas.mon / vas.mon / psl.mon"
    )
    parser.add_argument(
        "root",
        nargs="?",
        default=str(DEFAULT_ROOT),
        help=f"directory containing ERA5 nc files (default: {DEFAULT_ROOT})",
    )
    parser.add_argument(
        "--csv",
        default="era5_qc_summary.csv",
    ...
    for target in TARGETS:
        ...
        rows.append(analyze_target(root, target))
    ...
    if "status" in df.columns:
        ...
        df = df.sort_values(["_status_order", "label"]).drop(columns=["_status_order"])
    ...
    if len(bad) > 0:
        show_cols = [
            "status", "basename", "expected_var", "expected_freq", "detected_freq",
            "n_gap_steps", "n_missing_steps_est",
            "n_time_any_nan", "n_time_all_nan",
